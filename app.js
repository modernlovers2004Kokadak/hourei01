...
function restoreNavigationState(saved){
  ...
  else if(saved.screen==='lapMistakes')openLapMistakes();
...
function currentRoundHistoryRows(){
  const history=Array.isArray(prog.history)?prog.history.filter(row=>row&&qById.has(Number(row.id))&&typeof row.ok==='boolean'):[];
  const size=Math.max(1,LAW_QUESTIONS.length),fallback=Math.max(0,roundMeta().total)%size;
  const answered=roundHistory.current?Math.max(0,Math.min(size-1,Math.trunc(Number(roundHistory.current.answered)||0))):fallback;
  return answered?history.slice(-Math.min(answered,history.length)):[];
}
function currentRoundMistakeIds(){
  const seen=new Set(),ids=[];
  for(const row of currentRoundHistoryRows()){
    const id=Number(row.id);
    if(row.ok===false&&!seen.has(id)){seen.add(id);ids.push(id)}
  }
  return ids;
}
function renderHome(){
  setScreen('home');const all=totalStats(),rate=all.total?Math.round(all.correct/all.total*100):0,correctRate=all.total?all.correct/all.total*100:0,wrongRate=all.total?all.wrong/all.total*100:0,unansweredRate=all.total?all.unanswered/all.total*100:100,daily=todaySessionState(),mistakeCount=currentRoundMistakeIds().length,todayTitle=daily.completed?'本日完了':'今日の10問',todayCaption=daily.completed?`本日 ${daily.completedTotal}問 解答済み`:daily.total?`${daily.answered}／${daily.total}問 解答済み`:'毎日の法令確認';
  app.innerHTML=`<section class="home-card progress-card">
    ...
    <div class="today-wrap"><button class="today-start" onclick="LawBook.startToday()"><span class="check">✓</span><span><strong>${todayTitle}</strong><small>${todayCaption}</small></span></button><button class="today-mistakes today-side-button" onclick="LawBook.openLapMistakes()" aria-label="現在の周回で間違えた問題 ${mistakeCount}問"><span class="today-side-icon wrong-mark" aria-hidden="true">×</span><span class="count">${mistakeCount}</span></button><button class="today-bookmarks today-side-button" onclick="LawBook.openBookmarks()" aria-label="ブックマーク ${bookmarks.size}件"><span class="today-side-icon bookmark-mark" aria-hidden="true">🔖</span><span class="count">${bookmarks.size}</span></button></div>
    <div class="section-title-row"><h2 class="section-title">筆記試験科目</h2><a class="exam-guide-link" href="./preview.html?v=4.0.65">新制度による筆記試験実施要領</a></div>
...
function openLapMistakes(){
  setScreen('lapMistakes');
  const ids=currentRoundMistakeIds(),items=ids.map(id=>qById.get(id)).filter(Boolean);
  app.innerHTML=`${head('現在周回の間違えた問題')}<section class="lap-mistake-list">${items.length?items.map(q=>`<button class="bookmark-item lap-mistake-item" type="button" onclick="LawBook.startLapMistake(${q.id})"><span class="lap-mistake-question">${esc(q.q)}</span><span class="lap-mistake-answer"><small>正解</small><span>${esc(q.choices[q.answer]||'')}</span></span></button>`).join(''):'<div class="empty">現在の周回で間違えた問題はありません</div>'}</section>`;
}
function startLapMistake(id){
  const ids=currentRoundMistakeIds(),selected=Number(id);
  if(!ids.includes(selected)){openLapMistakes();return}
  startSession([selected,...ids.filter(value=>value!==selected)],'lapMistakes','lapMistakes');
}
...
function renderQuestion({preserveScroll=false}={}){
  ...
  const heading=sessionSource==='today'?'今日の10問':sessionSource==='lapMistakes'?'間違えた問題の解き直し':sessionSource==='retry'?'誤答の解き直し':sessionSource==='search'?'検索した問題':'関連問題';
...
function answer(index){const q=session[sessionIndex];if(sessionAnswers[q.id])return;const ok=index===q.answer;sessionAnswers[q.id]={choice:index,ok};answerNoticeId=q.id;if(sessionSource!=='lapMistakes')record(q,ok,index);saveTodaySession();renderQuestion({preserveScroll:true})}
...
function retryWrong(){const ids=session.filter(q=>sessionAnswers[q.id]&&!sessionAnswers[q.id].ok).map(q=>q.id);startSession(ids,sessionSource==='lapMistakes'?'lapMistakes':'retry',sessionSource==='lapMistakes'?'lapMistakes':'result')}
...
function back(){
  if(screen==='lapMistakes'){renderHome();return}
  if((screen==='question'||screen==='result')&&returnScreen==='lapMistakes'){openLapMistakes();return}
...
const LawBook={home,back,openSubject,openLaw,openRelatedLaw,openArticle,moveArticle,startToday,startNextToday,startRelated,startSubjectQuestions,startSession,startLapMistake,answer,previousQuestion,nextQuestion,retryWrong,toggleBookmark,openBookmarks,openLapMistakes,openLapHistory,openSourceArticle,openSearchLaw,openSearchArticle,openSearchQuestion,updateSearch,exportBackup,importBackup,resetSubject,resetLaw,resetAll,toggleText};
...
async function registerCurrentServiceWorker(){
  ...
  try{
    ...
    const registration=await navigator.serviceWorker.register('./sw.js?v=4.0.65',{updateViaCache:'none'});
